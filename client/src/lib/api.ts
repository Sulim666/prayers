...
export async function getLocationByIp() {
  const res = await apiRequest('GET', '/api/geo/ip-location', undefined);
  return res.json();
}
...
export async function getLocationByCoordinates(latitude: number, longitude: number, language = 'ru') {
  const res = await apiRequest(
    'GET', 
    `/api/geo/reverse-geocode?latitude=${latitude}&longitude=${longitude}&language=${language}`, 
    undefined
  );
  return res.json();
}
...
export async function searchLocations(query: string, language = 'ru') {
  const res = await apiRequest('GET', `/api/locations/search?query=${encodeURIComponent(query)}&language=${language}`, undefined);
  return res.json();
}
